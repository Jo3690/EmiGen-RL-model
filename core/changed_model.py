...
class GNNAsKernel(nn.Module):
    def __init__(self, nfeat_node, nfeat_edge, nhid, nout, nlayer_outer, nlayer_inner, gnn_types, 
        ...
        if task == "emi":
            ...
            self.output_decoder = torch.nn.Sequential(MLP(edge_emd_dim+nhid+state_dim, 128, nlayer=2, with_final_activation=False),MLP(128, nout, nlayer=2, with_final_activation=False),) # for fwhm
